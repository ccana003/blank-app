...
# === RULE 1–6: Administrative columns ===
...
if "q6" in df.columns and "q7" in df.columns:
    ...
    }).fillna(df.loc[q7_blank, "q7"])

# === RULE 9: q14b_ rename and logic ===
...
if "q14a" in df.columns and all(col in df.columns for col in q14b_fields):
    ...
    for col in q14b_fields:
        df.loc[condition & (df[col] == 0), col] = -1
...
# === RULE 13: Multiselect rename + OS logic ===
...
# === RULE 16: Exclusive multiselects ===
def apply_exclusive_logic(df, control_col, target_cols):
    if control_col in df.columns and all(col in df.columns for col in target_cols):
        ...
        mask_0 = df[control_col] == 0
        df.loc[mask_0, target_cols] = -1
        mask_1 = df[control_col] == 1
        none_selected = (df[target_cols] == 1).any(axis=1) == False
        mask_none_selected = mask_1 & none_selected
        df.loc[mask_none_selected, target_cols] = -9
    return df
...
# === FINAL ORDER ===
admin_columns = [f"admin_{i}" for i in range(1, 7)] + ["admin_6_os"]
